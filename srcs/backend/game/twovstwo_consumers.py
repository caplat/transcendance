...
class TwoVsTwoConsumer(GameConsumer):
    async def connect(self):
        await super().connect()
        print("2vs2 game mode activated")
    ...
    async def pads_init(self, game, width, height):
        ...
        padC = Pad('C', x=x, y=y * 3, h=height, w=width)
        padD = Pad('D', x=game.width - x, y=y * 3, h=height, w=width)
        ball = Ball(x = 400, y = 300, r = 5, speed = 5)

        if game.get_type() == "4players":
            padC.init_4players(game.width, game.height)
            padD.init_4players(game.width, game.height)
